...
const genDeck = (maxQuantity) => {
    let newArr = [];
    let typeALeft = 10;
    let typeBLeft = 10;
    let typeCLeft = 10;
    while (typeALeft > 0 && typeBLeft > 0 && typeCLeft > 0) {
        for (let i = 0; i < maxQuantity; i++) {

            let elem = arrCardTypes[randomInt(0, 2)];
            if (elem === 'a') typeALeft -= 1;
            if (elem === 'b') typeBLeft -= 1;
            if (elem === 'c') typeCLeft -= 1;
            newArr[i] = elem;
        }
    }
    return newArr;
...
//     let typeBLeft = 10;
//     let typeCLeft = 10;
//     arrCardTypes.reduce((acc, rec, index, array) => {
//         let temp = arrCardTypes[randomInt(0, 2)];
//         if (temp === 'a') typeALeft -= 1;
//         if (temp === 'b') typeBLeft -= 1;
//         if (temp === 'c') typeCLeft -= 1;
//         rec[index] = temp;
//         acc
//         return newArr;
//         // console.log(temp);
