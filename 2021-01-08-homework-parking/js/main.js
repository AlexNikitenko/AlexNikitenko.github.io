        ...
        tempEl.addEventListener('click', () => { // Ставим обработчик на клик по паркоместу
            ...
            if (elem.occupied === "Free") { //Проверяем свободно ли паркоместо
                ...
                tempBookedBtn.addEventListener('click', () => { //Ставим обработчик клика по кнопке ОК
                    ...
                    getParkingTime(elem.bookingTime);
...
const getParkingTime = (parkingTime) => { //Таймер для подсчета времени занятого места

    const timerID = setInterval(() => {
        let currTime = moment();
        let destTime = moment(`${parkingTime}`, 'HH:mm:ss');
        ...
        moment(diffTime.format('HH:mm:ss')).add(100, 'ms');
